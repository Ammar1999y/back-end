...
function FacetedTrigger(props: React.ComponentProps<typeof PopoverTrigger>) {
  ...
  return (
    <PopoverTrigger
      {...triggerProps}
      className={cn(
        'justify-between data-[state=open]:z-[1] data-[state=open]:ring-[2px] data-[state=open]:ring-ring/50',
        className
      )}
    >
      {children}
...
function FacetedBadgeList(props: FacetedBadgeListProps) {
  ...
  if (!values || values.length === 0) {
    return (
      ...
      >
        {placeholder}
        <ChevronDownIcon className='ms-auto size-4 text-muted-foreground opacity-80' />
      </div>
    );
  }
...
function FacetedContent(props: React.ComponentProps<typeof PopoverContent>) {
  ...
  return (
    <PopoverContent
      ...
      align='start'
      className={cn(
        'w-52 origin-[--radix-popover-content-transform-origin] p-0',
        className
      )}
    >
