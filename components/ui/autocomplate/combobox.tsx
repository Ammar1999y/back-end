...
import { badgeVariants } from '@/components/ui/autocomplate/badge';
import * as ComboboxPrimitive from '@/components/ui/autocomplate/combobox-primitive';
import {
  InputBase,
  ...
  InputBaseInput,
} from '@/components/ui/autocomplate/input-base';
  ...
  <ComboboxPrimitive.Anchor asChild>
    <InputBase ref={ref} {...props}>
      ...
      <ComboboxPrimitive.Trigger asChild>
        <InputBaseAdornmentButton className='transition-transform duration-300 data-[state=open]:rotate-180'>
...
>(({ children, icon, ...props }, ref) => (
  <ComboboxInputBase icon={icon}>
    <ComboboxPrimitive.ComboboxTagGroup asChild>
      <InputBaseFlexWrapper className='flex items-center space-x-2'>
        {children}
        <InputBaseControl>
          <ComboboxPrimitive.Input asChild>
            <InputBaseInput ref={ref} {...props} />
...
>(({ children, className, ...props }, ref) => (
  <ComboboxPrimitive.ComboboxTagGroupItem
    ...
    className={cn(
      ...
      'group pr-1.5 space-x-1 data-[disabled]:opacity-50',
      className
    )}
    {...props}
  >
...
>(({ className, align = 'center', alignOffset = 4, ...props }, ref) => (
  <ComboboxPrimitive.Portal>
    <ComboboxPrimitive.Content
      ...
      className={cn(
        'relative z-50 max-h-96 w-[--radix-popover-trigger-width] overflow-y-auto overflow-x-hidden rounded-md border bg-popover text-popover-foreground shadow-md data-[side=bottom]:translate-y-1 data-[side=left]:-translate-x-1 data-[side=right]:translate-x-1 data-[side=top]:-translate-y-1 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2',
        className
      )}
      autoFocus={false}
      {...props}
...
export const comboboxItemStyle = cva(
  'relative flex w-full cursor-pointer select-none items-center rounded-sm px-2 py-1.5 text-sm outline-none data-[disabled=true]:pointer-events-none data-[selected=true]:bg-accent data-[selected=true]:text-accent-foreground data-[disabled=true]:opacity-50'
);
