...
export type ComboboxContextProps = {
  ...
  tagGroupRef: React.RefObject<React.ElementRef<
...
export interface ComboboxTagGroupItemProps extends Omit<
...
>(({ onKeyDown, onMouseDown, onFocus, onBlur, ...props }, ref) => {
  ...
  const event = React.useCallback(
    ...
    [open, onOpenChange, type, inputValue, value, tagGroupRef, onValueChange]
  );
  return (
...
const findComboboxItemText = (children: React.ReactNode) => {
  ...
  React.Children.forEach(children, (child) => {
    ...
    if (React.isValidElement<{ children: React.ReactNode }>(child)) {
      text =
        child.type === ComboboxItemText
          ? (child.props.children as string)
          : findComboboxItemText(child.props.children);
    }
  });
