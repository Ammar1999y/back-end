...
import { CreateSectionInput } from '@/utils/validation/sections';
...
const ItemsSection = memo(() => {
  ...
  useEffect(() => {
    const desableTimeout = setTimeout(() => {
      useStore.getState().setDisabled(true);
    }, 300);
    return () => {
      clearTimeout(desableTimeout);
    ...
    <div className='relative space-y-4'>
      ...
      {fields.length === 0 ? (
        ...
        <SortableList
          ...
          overlay={
            activeItem ? (
              <ItemContent showDragHandle index={activeIndex} />
            ) : null
          }
        >
