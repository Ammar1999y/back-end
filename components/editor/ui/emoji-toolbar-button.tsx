...
export function EmojiToolbarButton({
  ...
  return (
    <EmojiPopover
      control={
        <ToolbarButton isDropdown pressed={isOpen} tooltip='إيموجي' {...props}>
          <SmileIcon />
        </ToolbarButton>
      }
      isOpen={isOpen}
...
function EmojiPickerSearchAndClear({
  ...
  return (
    <div className='flex items-center text-foreground'>
      <div
        className={cn(
          'absolute top-1/2 z-10 flex size-5 -translate-y-1/2 items-center justify-center text-foreground ltr:left-2.5 rtl:right-2.5'
        )}
      >
      ...
      {searchValue && (
        <Button
          aria-label='Clear'
          className={cn(
            'absolute top-1/2 flex size-8 -translate-y-1/2 cursor-pointer items-center justify-center rounded-full border-none bg-transparent text-popover-foreground hover:bg-transparent ltr:right-0.5 rtl:left-0.5'
          )}
          onClick={clearSearch}
          size='icon'
          title={i18n.clear}
