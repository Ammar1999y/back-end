...
<Callout type="warn">
  **This page runs ahead of the pinned version, and ahead of this deployment.**

`TOTP_ALREADY_ENABLED` does not exist in the installed `better-auth@1.7.2` — it appears nowhere under `node_modules/better-auth/dist/plugins/two-factor/`. In 1.7.2 a repeat `enable` silently REPLACES `secret` and `backupCodes`, so a verified authenticator is invalidated without saying so. (`allowPasswordless`, also documented here, IS present in 1.7.2.)

This deployment does not serve `/two-factor/enable`, `/two-factor/disable` or `/two-factor/generate-backup-codes` from the plugin at all: enrolment, removal and disable are owned in `lib/auth/two-factor-enrolment.ts`, where the refusal this callout describes is a real 409. Read this page for the plugin's model, not for the API this application exposes.
</Callout>
