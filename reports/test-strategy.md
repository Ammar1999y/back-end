# Test Strategy — what is left, and why the tests are shaped as they are

**Pruned 2026-08-28.** This was an implementation brief written when nothing was
implemented. Most of it has been built, so the instructions that were carried out
are gone. What remains is two things and nothing else:

- **Items still not implemented** — kept in full, with their `**Assert.**` and
  `**Seam.**` intact, because they are still the specification.
- **The rationale for tests that DO exist** — the `**Why.**` paragraph only.
  Six test files and `.github/workflows/ci.yml:85` cite this document by section
  for their justification, so those paragraphs are load-bearing prose, not history.

Deleted: everything already built, plus items settled the other way — the Better
Auth limiter statements (the tables were dropped instead), the log-boundary
circularity objection (closed differently, and better, by
`tests/unit/rate-limit-log-boundary.test.ts`), the OTP delivery-failure
accounting (reversed by design — delivery failure now refunds), and the
`/openapi.json`-is-public premise (the route is authenticated now).

**Claims in here that are now false about the code** — left in place rather than
silently rewritten, because each one is attached to an item that still needs
work, and correcting them is part of doing that work:

- §7.2f's readiness/sweep smoke coverage: both `/api/internal/*` routes are gone.
- §7.3a's `maintenanceTokenSet` readiness field: it does not exist; the token now
  gates the `?deep=1` branch instead.
...
| ------------------------------------------ | ----------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Test runner                                | `bun test` only                                                         | No vitest, no external assertion or mocking library. Coverage is built in.                                                                                    |
| Container orchestration                    | None locally                                                            | No `@testcontainers/postgresql`: it constructs under Bun but `.start()` hangs with no Docker daemon. CI's `services:` block is declarative and needs nothing. |
| PostgreSQL client                          | `bun:sql` + `drizzle-orm/bun-sql`, one pooled client in `db/index.ts`   | Session continuity exists, so `FOR UPDATE`, advisory locks and savepoints are all testable.                                                                   |
| Rate-limit / cache store                   | Local SQLite via `bun:sqlite`, owned fixed-window algorithm             | `now` is client-supplied, so window behaviour is drivable with `setSystemTime`. No network.                                                                   |
| Contract tests between two implementations | Dropped                                                                 | One implementation, tested directly.                                                                                                                          |
| Where destructive tests run                | Developer machine and CI only                                           | §3. The production VPS gets a read-only smoke set in a separate harness and nothing else.                                                                     |
| Database ownership                         | Tests own the database **because it is disposable**                     | Full ownership plus ephemeral is the norm; full ownership plus persistent-and-shared is the anti-pattern.                                                     |
...
What does belong on the server is a **read-only** post-deploy smoke set — health,
migration version, one known-good login — in its own harness, kept separate for a
specific reason: the day someone adds a `DELETE` to a suite that runs against
production, the separation stops protecting everything else too.

**CI has no persistent database, which is better than having one.** The job
migrates a `services:` container, runs the suite, and container and VM both
evaporate. Nothing persists, so a destructive suite has nothing to damage and
needs no cleanup logic. Pin the container's PostgreSQL major to the VPS's, or one
fidelity gap is traded for a smaller one rather than closed.

---

## 5. Standing rules for the suite

**`bun test <path>` treats its positional arguments as filename FILTERS, not
paths.** A test file outside the filter is skipped and the run still exits 0 —
which is how `bun test scripts/probe/local` came to report success while a whole
directory had never executed. `bunfig.toml`'s `[test] root` fixes the bare
`bun test` case; `tests/helpers/run.ts` fixes the per-tier case, and
`tests/unit/harness-layout.test.ts` asserts from the other side that no file
under `tests/` sits in no tier.

The runner owns the two jobs a preload cannot do: provision once and sequentially
(a preload runs per worker, and per file under `--isolate`, so `CREATE DATABASE`
there is N processes racing to clone one template), and drop once at the end
whatever happened (a preload's `afterAll` fires more than once per worker and
never at all when a worker is killed).

`test` stays the cheap tier deliberately: it is what `lefthook` runs, and a hook
that needs a database is a hook that gets disabled.
...
**Keep CI's `Boot smoke test` step exactly as it is**, including its
`DATABASE_URL` pointing at an unreachable host. It is the only thing proving the
pool still connects lazily (§7.4e); giving it a real database silently deletes
that assertion.

---

## 6. Facility rules worth keeping
...
| Genuine externality flakiness | `--retry=<N>`             | Only where the flakiness is a property of the world. A retry on a race assertion hides the race.                                                                                                                  |
| Time-dependent logic          | `setSystemTime`           | Restore with **`setSystemTime()` with no argument**. NOT `useRealTimers()` — measured on 1.4.0 it is `undefined` as a top-level `bun:test` export and exists only under the `jest`/`vi` compatibility namespaces. |
...
uppercase variant. **Not whitespace** — measured: `validID` calls `.trim()` before testing, so `<uuid>` is ACCEPTED and returns the canonical id. That is the safe direction (many spellings in, one out) and the opposite of the case behaviour, which is preserved — see `reports/claude-opus-audit.md` F27. `normalizeArabicDigits`
...
- **`sanitizeSvg` is not testable as a pure function under Bun.** With no injected
  ...
  EVERYTHING, so a table written against `sanitizeSvg(markup)` looks green while
