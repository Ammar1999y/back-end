...
| Fake timers                                       | `jest.useFakeTimers`, `setSystemTime`, `advanceTimersByTime`, `advanceTimersToNextTimer`, `runAllTimers`, `runOnlyPendingTimers`, `getTimerCount`, `clearAllTimers` — all present as top-level exports, driving both `setTimeout` and `Date`. `useRealTimers` is NOT among them: it exists only under `jest.`/`vi.`, so reach for `setSystemTime()` to restore the clock. `jest.unstable_mockModule` does **not** exist; `mock.module` is the seam. |
...
| Time-dependent logic          | `jest.useFakeTimers` + `setSystemTime` + `advanceTimersByTime`                       | Window rollover, the UTC-midnight OTP cap, DST, the six-hour block, cookie-cache staleness. Restore with **`setSystemTime()` with no argument** in `afterEach`. NOT `useRealTimers()` — measured on 1.4.0, it is `undefined` as a top-level `bun:test` export and exists only under the `jest`/`vi` compatibility namespaces, so the documented form here was a `ReferenceError` waiting to be written. 1.4 also stops fake timers leaking across files. |
...
- **There is no second copy.** `utils/svg/` does not exist — it was deleted, and
  with it the `TODO.md` EM-11 divergence. "Run the same table against both copies"
  sent a reader looking for a target that is not there.
