...
function AutoScroll(userOptions: AutoScrollOptionsType = {}): AutoScrollType {
  ...
  function createAutoScrollBehaviour(engine: EngineType): ScrollBodyType {
    ...
    function seek(): ScrollBodyType {
      // Check if there are enough snaps before proceeding
      ...
      const scrollTargetResult = scrollTarget.byDistance(0, false);
      if (!scrollTargetResult) return self;

      const currentIndex = scrollTargetResult.index;
