...
| Multipart               | **Unreachable.** Every write is one `PutObject` from a `Buffer` (`lib/media/upload.ts`): images are capped at `MAX_IMAGE_SIZE` = 1 MB and optimised to `SERVER_MAX_IMAGE_SIZE` = 0.2 MB, documents at `MAX_DOCUMENT_SIZE_MB` = 10 MB, and nothing uses the SDK's `Upload` helper.                                                                                                                                                                       |
